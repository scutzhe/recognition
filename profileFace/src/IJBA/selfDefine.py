...
def load_imgs(img_dir, image_list_file):
    ...
    with open(image_list_file, 'r') as imf:
        for line in imf:
            ...
            # print('img_path,yaw=',img_path)
            img_paths.append(img_path)
    return img_paths  #[img_path1,img_path2,...]
...
class CaffeCrop(object):
    ...
    def __call__(self, img):
        ...
        final_size = 224
        final_width = final_height = final_size # 224
        crop_size = 110
        crop_height = crop_width = crop_size # 110
        ...
        # computed parameters
        randint = rd.randint
        scale_height_diff = (randint(0,1000)/500-1)*scale_aug # 0
        crop_height_aug = crop_height*(1+scale_height_diff) # 110
        scale_width_diff = (randint(0,1000)/500-1)*scale_aug # 0
        crop_width_aug = crop_width*(1+scale_width_diff) # 110


        trans_diff_x = (randint(0,1000)/500-1)*trans_aug # 0
        trans_diff_y = (randint(0,1000)/500-1)*trans_aug # 0

        # center
        # crop_center_y_offset = 15; w*h = 31*38
        # crop_center_x_offset = 0
        center = ((img.width/2 + crop_center_x_offset)*(1+trans_diff_x),
                 (img.height/2 + crop_center_y_offset)*(1+trans_diff_y))

        if center[0] < crop_width_aug/2: #55
            crop_width_aug = center[0]*2-0.5 #75.5
            # print(1)
        if center[1] < crop_height_aug/2:#55
            crop_height_aug = center[1]*2-0.5 #37.5
            # print(2)
