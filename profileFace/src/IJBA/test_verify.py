...
def load_feat(list_file, bin_file):
    ...
    with open(list_file, 'r') as list_f, open(bin_file, 'rb') as bin_f:
        (data_num, feat_dim) = st.unpack('ii', bin_f.read(8)) # data_num int 3759, feat_dim int 256
        # feat_dim = bytes(str(feat_dim),encoding='utf8')
        for line in list_f:
            # print('line=',line,end = '')
            path = line.strip().split('/',4)[4]
            # print('path=',path)
            line_new = '/home/zhex/'+path
            # print('line_new=',line_new)
            record = line_new.strip().split('/')
...
def test_verify(model_type):
    ...
    #model_type = 'resnet18'
    protocol_dir = '/home/zhex/data/IJBA/protocol_11'
    align_img_dir = '/home/zhex/data/IJBA/align_image_11'
    tar1s, tar2s = [],[]
    split_num = 1
    for split in range(1, split_num+1):
        ...
        meta_data = load_meta_data(meta_file) #IJBA/protocol_11/split1/verify_metadata_1.csv

        # load extract feat
        feats = dict()
        feat_dim = 0
        split_img_dir = os.path.join(align_img_dir, 'ijb_a_11_align_split{}'.format(split))
        # split_img_dir = /home/zhex/data/IJBA/align_image_11/ijb_a_11_align_split1~10
