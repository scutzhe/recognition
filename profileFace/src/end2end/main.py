...
parser.add_argument('--train_list_file', default='/home/zhex/data/msceleb/train_list.txt', help='path to trainList')
parser.add_argument('--train_label_file', default='/home/zhex/data/msceleb/train_label.txt', help='path to trainLabel')
parser.add_argument('--val_list_file', default='/home/zhex/data/msceleb/val_list.txt', help='path to valList')
parser.add_argument('--val_label_file', default='/home/zhex/data/msceleb/val_label.txt', help='path to valLabel')
...
parser.add_argument('--end2end', action='store_true',
                    help='if true, using end2end with dream block, '
                         'else, using naive architecture')
...
def main():
    ...
    train_dataset =  MsCelebDataset(args.img_dir, args.train_list_file, args.train_label_file,
                                    transforms.Compose([caffe_crop,transforms.ToTensor()]))
    train_loader = torch.utils.data.DataLoader(train_dataset,batch_size=args.batch_size, shuffle=True,
                                               num_workers=args.workers, pin_memory=True)

    caffe_crop = CaffeCrop('test')
    val_dataset =  MsCelebDataset(args.img_dir, args.val_list_file, args.val_label_file,
            transforms.Compose([caffe_crop,transforms.ToTensor()]))
    val_loader = torch.utils.data.DataLoader(val_dataset,batch_size=args.batch_size, shuffle=False,
                                             num_workers=args.workers, pin_memory=True)

    assert(train_dataset.max_label == val_dataset.max_label),"error......"
    ...
    # prepare base model
    model = None
    assert(args.arch in ['resnet18','resnet34','resnet50','resnet101'])
    if args.arch == 'resnet18':
        model = resnet18(pretrained=False, num_classes=class_num, end2end=args.end2end)
    if args.arch == 'resnet34':
        model = resnet34(pretrained=False, num_classes=class_num, end2end=args.end2end)
    ...
    if args.arch == 'resnet152':
        model = resnet152(pretrained=False, num_classes=class_num, end2end=args.end2end)
...
def save_checkpoint(state, is_best, filename='checkpoint.pth'):
    ...
    full_bestname = os.path.join(args.model_dir, 'model_best.pth')
