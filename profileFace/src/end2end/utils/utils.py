...
def image_decode(image_path, color="RGB"):
    image_byte = open(image_path, 'rb').read()

    image = cv2.imdecode(np.asarray(bytearray(image_byte), dtype="uint8"), cv2.IMREAD_COLOR)
    if color == "RGB":
...
def make_weights_for_balanced_classes(images, nclasses):
    ...
    if isinstance(images[0], list) or isinstance(images[0], tuple):
        ...
        for item in images:
            count[item] += 1  # item is (img-data, label-id)
...
def separate_ir_bn_paras(modules):
...
def schedule_lr(optimizer, rate=0.1):
    for params in optimizer.param_groups:
        params['lr'] *= rate
