...
parser.add_argument('-ilf', '--image-list-file', default='/home/zhex/data/stitching/sample_img_list.txt',
        type=str, metavar='N', help='image list file')
parser.add_argument('-ff', '--feat-file', default='/home/zhex/data/stitching/ext_feat.bin',
...
parser.add_argument('--iters', default=5000000, type=int, metavar='N', help='number of total iters to run')
...
def main():
    ...
    for iter in range(args.iters):
        batch_train_feat, batch_target_feat, batch_norm_angle = get_batch(train_map, data, args.batch_size, args.feat_len)
        ...
        losses.update(loss.item(), loss.size(0))
        ...
        if iter !=0 and iter % 100000 == 0:
            torch.save({'state_dict': model.state_dict()}, './model/{}_checkpoint.pth'.format(iter))
...
def get_dict(file_name):
    ...
    with open(file_name) as f:
        for idx, line in enumerate(f):
            ...
            # st_align/A._J._DeLaGarza/5_0.jpg -29.7209 1
...
def get_batch(train_map, data, batch_size, feat_len):
...
class AverageMeter(object):
    # inherent object is new class and not is classic class
